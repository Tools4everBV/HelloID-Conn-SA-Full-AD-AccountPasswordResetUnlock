...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD Account - Reset password & Unlock" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-key" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
...
'@; 
	...
	$tmpVariables = @'
[{"name":"blnreset","value":"{{form.blnreset}}","secret":false,"typeConstraint":"string"},{"name":"blnunlock","value":"{{form.blnunlock}}","secret":false,"typeConstraint":"string"},{"name":"password","value":"{{form.password}}","secret":false,"typeConstraint":"string"},{"name":"userPrincipalName","value":"{{form.gridUsers.UserPrincipalName}}","secret":false,"typeConstraint":"string"}]
'@ 

	$delegatedFormTaskGuid = [PSCustomObject]@{} 
	Invoke-HelloIDAutomationTask -TaskName "AD-user-reset-unlock" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD Account - Reset password & Unlock' already exists. Nothing to do with the Delegated Form task..." 
}
